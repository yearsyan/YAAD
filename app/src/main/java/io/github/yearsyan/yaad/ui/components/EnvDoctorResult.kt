...
suspend fun handleYtDlpResult(context: Context) {
    ...
    if (!YtDlpUtil.downloadVersion(context, version)) {
        PopTip.show(com.kongzue.dialogx.R.mipmap.ico_dialogx_error, context.getString(R.string.net_error))
        return
    }
    if (!YtDlpUtil.isYtDlpExists(context)) {
        PopTip.show(com.kongzue.dialogx.R.mipmap.ico_dialogx_error, context.getString(R.string.download_fail))
        return
    }
}
