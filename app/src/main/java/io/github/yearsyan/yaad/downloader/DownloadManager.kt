...
object DownloadManager : IDownloadListener {
    enum class DownloadType { SINGLE_HTTP, BT, EXTRACTED_MEDIA, CHILD_HTTP }
    ...
    class SingleHttpDownloadSessionRecord(
        ...
        originLink: String,
        recoverFile: String
    ) : DownloadSessionRecord(sessionId, DownloadType.SINGLE_HTTP, originLink, recoverFile) {
        ...
        override fun isActiveForService(): Boolean {
            ...
            return jobContext?.isActive == true && (state == DownloadState.DOWNLOADING || state == DownloadState.VALIDATING || state == DownloadState.PENDING)
        }
    }
    ...
    private val dispatcher = ThreadPoolExecutor(
        4, 32, 30L, TimeUnit.SECONDS, LinkedBlockingQueue()
    ).asCoroutineDispatcher()
    private val downloadScope = CoroutineScope(dispatcher + SupervisorJob())

    private val _tasksFlow = MutableStateFlow<List<DownloadSessionRecord>>(emptyList())
    ...
    private val sessionMap = ConcurrentHashMap<IDownloadSession, WeakReference<DownloadSessionRecord>>()

    private var isServiceRunning = false
    private val serviceControlMutex = Mutex()

    fun initByApplication(app: Application) {
        if (Looper.getMainLooper() != Looper.myLooper()) throw RuntimeException("Must be called from main thread")
        context = app
    ...
    fun addHttpDownloadTask(
        ...
        val httpSession = HttpDownloadSession(url = url, path = fileDist.absolutePath, headers = headers)

        httpSession.addDownloadListener(this)
        val job = downloadScope.launch(Dispatchers.IO) { httpSession.start({
            startResultListener(it)
            checkAndControlService()
        }) }

        val record = SingleHttpDownloadSessionRecord(sessionId, url, httpSession.recoverFilePath())
    ...
    suspend fun deleteDownloadTask(sessionId: String) {
        val record = synchronized(downloadTasks) {
            downloadTasks.find { it.sessionId == sessionId } ?: return
        }
        
        val session = (record as? SingleHttpDownloadSessionRecord)?.httpDownloadSession
    ...
    private fun notifyListeners(session: IDownloadSession, callback: IDownloadTaskListener.(IDownloadSession, DownloadSessionRecord) -> Unit) {
        val record = sessionMap[session]?.get() ?: return
        synchronized(downloadListeners) {
            downloadListeners.forEach { listener -> callback(listener, session, record) }
        }
    }

    private fun updateFlow() {
        synchronized(downloadTasks) { _tasksFlow.value = downloadTasks.toList() }
    }
            ...
            return downloadTasks.any { record ->
                when (record) {
                    is SingleHttpDownloadSessionRecord -> {
                        ...
                        val state = session?.getStatus()?.state
                        session != null && state != null && (state == DownloadState.DOWNLOADING || state == DownloadState.PENDING)
                    }
                    else -> false
                }
            }
            ...
            serviceControlMutex.withLock {
                ...
                if (shouldServiceRun && !isServiceRunning) {
                    val serviceIntent = Intent(context, DownloadForegroundService::class.java).apply {
                        action = DownloadServiceConstants.ACTION_START_FOREGROUND_SERVICE
                    }
                    ContextCompat.startForegroundService(context, serviceIntent)
                    isServiceRunning = true
                } else if (!shouldServiceRun && isServiceRunning) {
                    val serviceIntent = Intent(context, DownloadForegroundService::class.java).apply {
                        action = DownloadServiceConstants.ACTION_STOP_FOREGROUND_SERVICE
                    }
                    context.stopService(serviceIntent) // stopService is fine, it will call onDestroy in service
