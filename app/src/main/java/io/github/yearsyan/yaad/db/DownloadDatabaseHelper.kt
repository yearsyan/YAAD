...
class DownloadDatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun saveDownloadSession(record: DownloadSessionRecord) {
        ...
        db.insertWithOnConflict(TABLE_DOWNLOAD_SESSIONS, null, values, SQLiteDatabase.CONFLICT_REPLACE)
    }

    fun deleteDownloadSession(sessionId: String) {
        val db = this.writableDatabase
        db.delete(TABLE_DOWNLOAD_SESSIONS, "$COLUMN_SESSION_ID = ?", arrayOf(sessionId))
    }
    ...
    fun getAllDownloadSessions(): List<DownloadSessionRecord> {
        ...
        cursor.use {
            while (it.moveToNext()) {
                ...
                val downloadType = DownloadType.valueOf(it.getString(it.getColumnIndexOrThrow(COLUMN_DOWNLOAD_TYPE)))
                ...
                val record = when (downloadType) {
                    DownloadType.SINGLE_HTTP -> SingleHttpDownloadSessionRecord(sessionId, originLink, recoverFile)
                    else -> DownloadSessionRecord(sessionId, downloadType, originLink, recoverFile)
                }
                records.add(record)
            }
        }
