...
open class RepoRelease {
    ...
    private val releaseUrl: String get() {
        return ""
    }
    ...
    fun downloadVersion(scope: CoroutineScope, context: Context, version: String, downloadCb: (Float) -> Unit = {}): Job {
        val downloadUrl = getReleaseDownloadUrl(version)
        val outputFile = File(context.filesDir, YT_GET_FILE_NAME)
        val session = HttpDownloadSession(
            url = downloadUrl,
